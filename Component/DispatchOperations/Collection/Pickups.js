...
import CToperations from "./CToperation";
...
export default function Pickups({navigation}) {
 ...
 const openPickup=(e)=>{
  ...
  return (
    ...
    <HeaderComp iconName={'cubes'} name={"Assignment"} nav={navigation}/>
     ...
     <TouchableOpacity onPress={()=>dateFilterRef.current.open()} style={{flexDirection:'row',alignItems:'center',marginTop:10}}><IconComp name={"calendar-week"} color={AppColor.third} size={20} /></TouchableOpacity>
      ...
      <Custombtm   e={()=><Parcel parcelId={appOp.parcelId}/>} height={Dimensions.get('screen').height} btmRef={parcelRef} cod={false}/>
       ...
       <Custombtm e={()=><CToperations pickup={appOp.pickupObject} onChange={()=>onOpeChange()}/>} height={Dimensions.get('screen').height} btmRef={pickRef} cod={true}/>
